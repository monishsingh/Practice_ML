# -*- coding: utf-8 -*-
"""
Created on Thu Apr 12 03:48:19 2018
...
if __name__ == "__main__":
    ...
    # no_of_input = m.floor(len(lines)*0.80)
     ...
     Y = np.zeros(shape=(no_of_training,2))
     ...
     W3 = np.random.randn(50,2) * np.sqrt(2/100)  
     
     Z3 = act_layer2.dot(W3)
     act_layer3 = np.zeros(shape=(no_of_training,2))
     for i in range(0,no_of_training):
         for j in range(0,2):
             act_layer3[i][j] = softmax(Z3[i][j],Z3[i])
    ...
    # delta4 = act_layer3 - Y
     ...
     for i in range(0,9):
       ...
       # W3 = np.random.randn(50,1) * np.sqrt(2/100)  
        ...
        #####################################################
        output1 = np.zeros(shape=(len(act_layer3),2))
     
        for i in range(0,len(act_layer3)):
            for j in range(0,2):
             
                if float(act_layer3[i][j]) < 0.4:
                    output1[i][j] = 0
        ...
        for i in range(0,len(output1)):
            for j in range(0,2):
                 ...
                 if float(output1[i][j]) == float(Y[i][j]):
                      a = 1
                      if j==1:
     ...
     file = open("insample_error_softmax.txt",'w')
     ...
     Y_test = np.zeros(shape=(len(test_lines),2))
     for i in range(0,len(test_lines)):
    ...
    # W3 = np.random.randn(50,1) * np.sqrt(2/100)  
          ...
          act_layer3_test = np.zeros(shape=(len(test_lines),2))
          for i in range(0,len(test_lines)):
              for j in range(0,1):
                  act_layer3_test[i][j] = softmax(Z3_test[i][j],Z3_test[i])
             
      ########################## accuracy####################
    
     output_test = np.zeros(shape=(len(test_lines),2))
     
     for i in range(0,len(test_lines)):
         for j in range(0,2):
             
             if float(act_layer3_test[i][j]) < 0.4:
                 output_test[i][j] = 0
             else:
                 output_test[i][j] = 1
     ...
     for i in range(0,len(test_lines)):
         for j in range(0,2):
             ...
             if float(output_test[i][j]) == float(Y_test[i][j]) :
                 total = total + 1
     ...
     accuracy = (total/(len(test_lines)*2))*100
     print(accuracy)
    ...
    ######################## 1st iteration #####################
     ...
     file = open("outsample_error_softmax.txt",'w')
